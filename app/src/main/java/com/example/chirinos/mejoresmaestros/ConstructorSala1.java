package com.example.chirinos.mejoresmaestros;

public class ConstructorSala1 {
    private String lectura, encargado1, ayudante1, encargado2, ayudante2, encargado3, ayudante3, evento, tipo1, tipo2, tipo3;
    private Integer semana, diaDiscurso, mesDiscurso, anualDiscurso;

    public ConstructorSala1() {}

    public String getLectura() {
        return lectura;
    }

    public String getEncargado1() {
        return encargado1;
    }

    public String getAyudante1() {
        return ayudante1;
    }

    public String getEncargado2() {
        return encargado2;
    }

    public String getAyudante2() {
        return ayudante2;
    }

    public String getEncargado3() {
        return encargado3;
    }

    public String getAyudante3() {
        return ayudante3;
    }

    public String getEvento() {
        return evento;
    }

    public String getTipo1() {
        return tipo1;
    }

    public String getTipo2() {
        return tipo2;
    }

    public String getTipo3() {
        return tipo3;
    }

    public Integer getSemana() {
        return semana;
    }

    public Integer getDiaDiscurso() {
        return diaDiscurso;
    }

    public Integer getMesDiscurso() {
        return mesDiscurso;
    }

    public Integer getAnualDiscurso() {
        return anualDiscurso;
    }

    public void setLectura(String lectura) {
        this.lectura = lectura;
    }

    public void setEncargado1(String encargado1) {
        this.encargado1 = encargado1;
    }

    public void setAyudante1(String ayudante1) {
        this.ayudante1 = ayudante1;
    }

    public void setEncargado2(String encargado2) {
        this.encargado2 = encargado2;
    }

    public void setAyudante2(String ayudante2) {
        this.ayudante2 = ayudante2;
    }

    public void setEncargado3(String encargado3) {
        this.encargado3 = encargado3;
    }

    public void setAyudante3(String ayudante3) {
        this.ayudante3 = ayudante3;
    }

    public void setEvento(String evento) {
        this.evento = evento;
    }

    public void setTipo1(String tipo1) {
        this.tipo1 = tipo1;
    }

    public void setTipo2(String tipo2) {
        this.tipo2 = tipo2;
    }

    public void setTipo3(String tipo3) {
        this.tipo3 = tipo3;
    }

    public void setSemana(Integer semana) {
        this.semana = semana;
    }

    public void setDiaDiscurso(Integer diaDiscurso) {
        this.diaDiscurso = diaDiscurso;
    }

    public void setMesDiscurso(Integer mesDiscurso) {
        this.mesDiscurso = mesDiscurso;
    }

    public void setAnualDiscurso(Integer anualDiscurso) {
        this.anualDiscurso = anualDiscurso;
    }
}
